...
double evaluate(char *expression)
{
    ...
    while (expression[i] != '\0')
    {
        ...
        if (isSymbol(expression[i]))
        {
            ...
            top:
                ...
                if (Stack_operator >= Current_operator)
                {
                ...
                {
                    push(&operator,(double) expression[i]);
                }
            }
    ...
    // printf("%c is operator and %d is top of the operator stack",operator.data[operator.top],operator.top);

    return operand.data[operand.top];
}
